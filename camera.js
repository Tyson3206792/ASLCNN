function mode(func){	//Runs when function buttons are pressed
...
async function collectImage(){	//Manages camera layout and image collection for different landscape/portrait/front/back camera combinations
    context.beginPath();
    ...
    if(video.videoWidth < video.videoHeight){   //Portrait mode
        ...
        {
            context.scale(-1, 1);
            context.drawImage(video, 0, -140, -480, 640);    //Put image within 200/200 box and process whole thing
            context.beginPath();
            context.strokeStyle = "#0000FF";
            context.strokeRect(-260, y, -200, 200);
            context3.drawImage(video, -20, -y-140, 480, 640);
            imgData = context3.getImageData(0, 0, 200, 200);
        }
        else
        {
            context.drawImage(video, 0, -140, 480, 640);
            context.beginPath();
            context.strokeStyle = "#0000FF";
            context.strokeRect(20, y, 200, 200);
            imgData = context.getImageData(20, y, 200, 200);   //Bounded image
        }
    }
    else{       //Landscape mode
        canvas.width = 640;
...
async function runDetection() {		//Main loop function
...
function graph(a, v){		//Displays predictions
...
function countDown(){	//Manage and display countdown timer for learn function
	clearInterval(timer);
	seconds = 60;
...
function mobilecheck() {	//Returns true if running on mobile
  var check = false;
...
var canvas2 = document.getElementById("mycanvas2");		//Stores prediction graph
var context2 = canvas2.getContext("2d");
var canvas3 = document.getElementById("mycanvas3");		//Stores image to be processed
...
function access_webcam(){		//Swap between front and rear camera
